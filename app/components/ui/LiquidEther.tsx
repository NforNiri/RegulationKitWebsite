...
class Output {
    ...
    camera: THREE.Camera;
    output: THREE.Mesh;
    constructor(paletteTex: THREE.DataTexture, bgVec4: THREE.Vector4) {
...
class WebGLManager implements LiquidEtherWebGL {
    ...
    constructor(props: any, paletteTex: THREE.DataTexture, bgVec4: THREE.Vector4) {
        this.props = props;
        ...
        this._onVisibility = () => {
            ...
            if (hidden) {
                this.pause();
            } else if (this.isVisibleRef && this.isVisibleRef.current) {
                this.start();
            }
        };
    ...
    init(paletteTex: THREE.DataTexture, bgVec4: THREE.Vector4) {
        ...
        this.output = new Output(paletteTex, bgVec4);
    }
    resize() {
        Common.resize();
        this.output.resize();
...
export default function LiquidEther({
    ...
    useEffect(() => {
        ...
        webgl.isVisibleRef = isVisibleRef;
    ...
    useEffect(() => {
        ...
        if (webgl.autoDriver) {
            webgl.autoDriver.enabled = Boolean(autoDemo);
            webgl.autoDriver.speed = Number(autoSpeed);
            webgl.autoDriver.resumeDelay = Number(autoResumeDelay);
            webgl.autoDriver.rampDurationMs = Number(autoRampDuration) * 1000;
            if (webgl.autoDriver.mouse) {
                webgl.autoDriver.mouse.autoIntensity = Number(autoIntensity);
                webgl.autoDriver.mouse.takeoverDuration = Number(takeoverDuration);
            }
        }
        if (resolution !== prevRes) sim.resize();
    }, [
